...
const readMess = new WeakSet(); // хранит уникальные сообщения

readMess.add(messages[0]);
readMess.add(messages[0]);
// дважды добавленное сообщение, появится в weakSet только один раз - что нам и нужно
...
/** 
 * map использует объекты в виде ключа, 
 * что позволяет нам хранить информацию об этом объекте.
 * Например дату прочтения, как в этом примере
 */
readDates.set(messages[0], Date.now()) 